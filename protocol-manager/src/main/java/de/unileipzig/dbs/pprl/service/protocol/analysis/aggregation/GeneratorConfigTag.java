...
public class GeneratorConfigTag extends TaggedResultAggregator {

  public static final String CRITERIA = "Diff tag";

  private static List<String> generatorConfigTags = null;
...
//    System.out.println(tmp.printAll());
  ...
  public static Table filterByTagTable(Table byTag) {
    if (generatorConfigTags == null) {
      generatorConfigTags = Arrays.stream(DataSetModifierConfigProvider.GeneratorConfigTag.values())
        .map(Enum::name)
        .collect(Collectors.toList());
    }
    return byTag.where(byTag.stringColumn(LINK_TAG).isIn(generatorConfigTags)).sortOn(0, 1);
  }
