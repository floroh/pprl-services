...
import de.unileipzig.dbs.pprl.core.analyzer.tags.record.RecordLengthAnalyzer;
...
public class RecordLength extends TaggedResultAggregator {

  public static final String CRITERIA = "Record length";
  ...
  public Table aggregateFromByTag(Table byTag) {
    Table tmp = byTag.where(
      byTag.stringColumn(LINK_TAG).containsString(RecordLengthAnalyzer.RECORD_LENGTH)
//        .or(byTag.stringColumn(LINK_TAG).containsString(LEN))
    ).sortOn(0, 1);
...
//    tmp.forEach(r -> {
//      String lengthTag = r.getString(LINK_TAG);
//      String name = StringUtils.substringBefore(lengthTag, LEN);
//      if (!stats.containsKey(name)) {
//        stats.put(name, new DescriptiveStatistics());
//      }
//      int length = Integer.parseInt(StringUtils.substringAfter(lengthTag, LEN));
//      stats.get(name).addValue(length);
//    });
//
//    // Get values for n-SHORTEST and n-LONGEST
//    Table rounded = tmp.emptyCopy();

    // Replace link tags with categorized length tags (SHORT, AVERAGE, LONG)

    Table result = getResultTable(
      StringColumn.create(CRITERIA)
    ).setName("ByRecordLength");
