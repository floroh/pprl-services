...
import static de.unileipzig.dbs.pprl.core.analyzer.tags.recordpair.PlainRecordPairAnalyzer.*;
import static de.unileipzig.dbs.pprl.core.common.monitoring.TagTable.LINK_TAG;


public class AttributeEqual extends TaggedResultAggregator {
  ...
  public Table aggregateFromByTag(Table byTag) {
    Table tmp = byTag.where(
      (byTag.stringColumn(LINK_TAG).containsString(EQUAL)
        .andNot(byTag.stringColumn(LINK_TAG).containsString(ALL_EQUAL)))
      .or(byTag.stringColumn(LINK_TAG).containsString(UNEQUAL))
    ).sortOn(0, 1);
//        System.out.println(tmp.printAll());

    Table result = getResultTable(
      StringColumn.create(CRITERIA)
    ).setName("ByAttributeEqual");
