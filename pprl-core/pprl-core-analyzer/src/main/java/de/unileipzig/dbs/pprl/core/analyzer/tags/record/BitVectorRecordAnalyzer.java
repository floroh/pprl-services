...
public class BitVectorRecordAnalyzer implements RecordAnalyzer {

  public static final String TAG_PREFIX = "ENC";
  public static final String MISSING = TAG_PREFIX + "_MISSING";

  private final BitVectorCardinalityAnalyzer bitVectorCardinalityAnalyzer = new BitVectorCardinalityAnalyzer();
  ...
  public List<Tag> getTags(Record record) {
    ...
    for (String attrName : record.getAttributeNames()) {
      ...
      if (!(attr.getType().equals(Attribute.Type.BITVECTOR))) {
        continue;
      }
      BitVector bv = attr.getAs(BitVector.class);

      //Cardinality
      bitVectorCardinalityAnalyzer.getTags(attrName, bv).forEach(tag -> tags.add(tag.addID(record)));
    }
    return tags;
  }
}
